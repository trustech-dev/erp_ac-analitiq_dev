...
class HrExpenseSheet(models.Model):
    ...
    analytic_account = fields.Many2one(
        "account.analytic.account", related="project_id.analytic_account_id"
    )
    ...
    def write(self, vals):
        ...
        if vals.get("payment_status"):
            if bool(vals["payment_status"]) and (
                not bool(
                    self.env.user.has_group("mdx_analitq.group_medex_payment_status")
                )
            ):
                raise UserError("You cannot edit the Payment Status.")
        if any(state == "draft" for state in set(self.mapped("state"))):
            ...
            raise UserError("No edit in this state")
    ...
    def action_submit_sheet(self):
        ...
        if len(self.expense_line_ids.ids) == 0:
            raise UserError("Expense line cannot be empty.")
        if attachment_count > 0:
            for attachment in attachments:
                file_category = attachment.mimetype.split("/")[0]
                if file_category != "image":
                    raise UserError("Upload receipt images in image format.")
